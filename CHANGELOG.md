...
- Added a dedicated non-Life accessibility/performance hardening layer across all 25 public UI-4 surfaces.
- Certified heading semantics, alt/decorative image policy, labeled controls, keyboard/skip focus, AA-safe gold text, reduced motion, forced colors, and 320px / 400%-equivalent reflow.
- Added intrinsic image dimensions everywhere on the non-Life public surface and fixed footer grid min-content behavior revealed by that reservation.
- Added narrow responsive WebP candidates for all four Professional Program hero portraits and Local context photography.
- Reduced the Home + Auto 320px editorial hero from ~74 KiB to ~45 KiB and added 506px WebP 408FARMERS logo candidates.
- Enforced sole hero fetch priority on image-led primary routes and removed competing high priority from the Home header logo.
- Preserved protected forms, Workers, routing, attribution, Local behavior, CoverageFit contracts, Life and Life Ops.
- Documented `/home/` at 323.2 KiB uncompressed CSS+JS under a 330 KiB UI-4 ceiling; the historical UI-3.13 300 KiB check is the sole accepted legacy exception.
- Next: 408-UI-4.10 — Functional Regression + Production Certification.


## 408-UI-4.7 — Relationship + Completion Editorial Convergence
- Added the UI-4 editorial completion layer across dynamic post-lead, CoverageFit invitation, receipts, Contact, Neighbor, Score and utility surfaces.
- Preserved all protected forms, workflow runtimes, Life/Life Ops, Local independence and the 408-INFRA-1.1 function boundary.
# 408-UI-3.13.1A — Human Trust Foundation

## 2026-08-19 — 408-UI-3.13.1C Core Insurance Humanization
- Applied the restrained Human Trust layer to Homepage, Home, Home + Auto, and Buyer.
- Added earlier Dylan identity, warmer consultative/concierge copy, and selective de-chroming without changing core form/runtime contracts.
- Preserved Professional Programs 3.13.1B, Life/Life Ops, Local, Campaign Entry Registry, CoverageFit, Workers, and the 408-INFRA-1.1 boundary.


- Added an opt-in Human Trust design layer for all non-Life public 408FARMERS surfaces.
- Established reusable Dylan signature, editorial reassurance, contextual photography, Professional Programs gold-accent, Local cue, and first-person completion primitives.
- Added `HUMAN-TRUST-DESIGN-SYSTEM.md` and machine-readable `HUMAN-TRUST-COMPONENT-REGISTRY.json`.
- Loaded the inert foundation stylesheet on 25 non-Life public HTML surfaces without changing their body markup or current visual composition.
- Preserved all Life/Life Ops files and the unrelated 408-INFRA-1.1 Pages Function Invocation Boundary Hotfix byte-for-byte.
- Locked the next refinement as 408-UI-3.13.1B — Professional Programs Humanization.

# 408-UI-3.12 — End-to-End Functional Regression

- Added a dedicated 59/59 cross-journey Playwright regression suite using the shipped page/runtime scripts with deterministic endpoint mocks.
- Reverified Home, Home + Auto, Buyer, Professional Programs, Life, Local, merchant join, campaign matching, attribution, QR/Worker routing, and explicit CoverageFit continuation.
- Captured behavior-critical and expanded product-surface hash freezes and confirmed UI-3.12 changes no customer-facing/runtime/static product file.
- Reran the latest applicable UI-3 browser, mobile, Worker, static, routing, submit, logo, and internal-link suites.
- Preserved the LOCAL-1.10 planned three-merchant production NO-GO boundary.
- Advanced the UI roadmap to UI-3.13 — Production Design Certification.

# 408-UI-3.11.1 — Campaign Message-Match Convergence

- Added a current-entry-only campaign presentation registry and runtime.
- Matched Stevie's coaster front/back, occupational Meta, Buyer/Realtor, and existing Life campaign entry states to their certified UI-3 destinations.
- Kept dynamic ZIP Home flyers delegated to the established flyer renderer.
- Added campaign registry, message matrix, and creative refresh specification.
- Preserved existing runtime, Worker, form, attribution, secure Life, Local, and CoverageFit behavior.
- Unknown or stale campaign context fails safely to evergreen route copy.

# 408-UI-3.11 — Accessibility Certification

- Certified all 27 public consumer UI-3 surfaces against the project WCAG 2.2 AA-oriented accessibility contract.
- Added a final high-contrast focus layer, stronger form-control/secondary-action boundaries, readable placeholder contrast, and a Life dark-canvas small-accent correction.
- Added native invalid-state ARIA reflection, atomic status announcements, and Escape focus return for the mobile navigation without suppressing native validation or changing submission behavior.
- Certified semantic hierarchy, labels, image alternatives, ARIA references, reduced motion, forced colors, user zoom, and narrow-screen/400%-zoom reflow guards.
- Preserved all pre-existing runtime JavaScript, `_worker.js`, Local catalog/runtime, Life secure submission, attribution, form contracts, QR routing, and CoverageFit handoffs.

# 408-UI-3.9 — Utility + Completion Surfaces

- Converged fallback receipts, Contact, Privacy, Terms, Local merchant-join completion, the neighbor/CoverageFit handoff, and shared error/empty/recovery states with the UI-3 platform language.
- Preserved Life completion in the Life campaign environment while aligning completion hierarchy and action styling.
- Added a branded static `404.html` fallback.
- Repaired malformed legacy next-step markup on five fallback receipts without changing user-facing copy or flow behavior.
- Preserved Workers, submissions, attribution, Local, Life secure submission, QR routing, and CoverageFit contracts.

# 408-UI-3.8 — Local Visual Convergence

- Converged `/local/`, dynamic Local merchant detail/redemption, and `/local/join/` with the compact UI-3 platform aesthetic.
- Promoted the live merchant directory immediately after the Local intro.
- Added `shared/local-visual-convergence.css` with restrained cards, filters, merchant detail, redemption, join-form, responsive, and accessibility treatments.
- Preserved Local catalog/runtime/attribution/redemption/join/Worker behavior and the LOCAL-1.10 operational NO-GO boundary.
- Added no distance, ratings/reviews, geolocation, fabricated merchant imagery, accounts, or unsupported Local functionality.


## 408-UI-3.7 — Life Campaign + Platform Convergence
- Preserved the LIFE campaign's cinematic black / white / red identity within the unified UI-3 platform shell.
- Added campaign family artwork and a dedicated Life visual convergence stylesheet.
- Tightened existing A/B/C visible message matching to mirror the approved paid-social creative while keeping campaign keys, creative codes, attribution and conversion measurement unchanged.
- Reframed the secure application as a white working surface inside the dark campaign environment.
- Preserved all secure submission, producer queue, privacy, Worker, and non-Life route behavior.

## 408-UI-3.5 — Buyer Experience Redesign
- Aligned `/buyer/` and the buyer fallback receipt with the UI-3 platform visual system.
- Added `shared/buyer-experience-ui.css` with a closing-first split hero, restrained guided-intake card, Farmers-red primary actions, compact producer credibility, and responsive/mobile treatment.
- Preserved buyer/realtor referral context, closing urgency, text-first SMS entry, field inventory, submission, post-lead behavior, and CoverageFit handoff contracts.
- No Worker, Buyer runtime, attribution runtime, or CoverageFit sender logic changed.
# 408-UI-3.2 — Homepage Platform Redesign

- Reframed `/` as a compact 408FARMERS platform hub using the UI-3.1 navy/red/white system.
- Added clear primary routes for Home, Home + Auto, Buyers, and Life plus compact Protection Score, Business, and Rental Property entry points.
- Converged Professional Programs, CoverageFit explanation, Local, and Dylan/agency credibility into one restrained homepage hierarchy.
- Preserved every pre-existing tracked homepage destination and CoverageFit launch contract.
- Added `shared/homepage-platform.css`; protected Workers, acquisition runtimes, Local, Life, Buyer, QR, campaign, and handoff logic remain unchanged.
- Advanced the UI roadmap to 408-UI-3.3 — Home Conversion Experience.

# 408-UI-3.1 — Unified Visual Foundation

- Added one sitewide navy/red/white 408FARMERS visual foundation.
- Added compact universal header, mobile navigation, and universal footer.
- Added shared typography, spacing, radii, borders, shadows, buttons, form controls, cards, and status treatments.
- Added a high-contrast 408FARMERS navigation-logo asset for the navy header.
- Loaded the foundation last on 26 public HTML surfaces while leaving internal QA fixtures and life-ops outside consumer styling.
- Preserved all underlying form, Worker, attribution, Local, Life, QR, campaign, and CoverageFit contracts.
- Added UI-3.x roadmap and UI-3.1 behavior-preservation certification.

# 408-LOCAL-1.10 — Conversion + Compliance Certification

- Completed the Local MVP technical/source certification matrix across routing, lifecycle states, offer timing/terms, redemption independence, mobile/reflow/accessibility, QR routing, attribution persistence, privacy-safe telemetry, insurance-bridge separation, and existing 408FARMERS regressions.
- Added automated browser QA for the Local directory, Stevie's merchant detail/redemption experience, and merchant join flow at phone, tablet, and desktop widths.
- Added a California advertising/licensing preflight note using public CDI/Farmers sources while explicitly preserving the requirement for internal agency/Farmers approval.
- Added Virginia Tam Insurance Agency, Inc. · CA License #4528400 to the Stevie's printable Local placard as a conservative print-disclosure safeguard.
- Did not fabricate Auto or Home pilot merchants. The planned three-merchant MVP therefore receives a documented production **NO-GO** until operational 1.9 closeout, physical scan/staff checks, and internal advertising approval are complete.
- Preserved Local merchant catalog/runtime, attribution runtime, join flow, Worker routing, all insurance conversion flows, CoverageFit, and insurance economics.
- Final certification artifacts: `LOCAL1_10_RELEASE_CERTIFICATION.json`, `LOCAL1_10_QA.json`, `LOCAL1_10_BROWSER_QA.json`, `LOCAL1_10_QR_QA.json`, `LOCAL1_10_LINK_QA.json`, and `LOCAL1_10_JS_SYNTAX_QA.json`.

# 408-LOCAL-1.9 — Pilot Merchant Launch

- Activated Stevie's Bar & Grill as the first real public 408FARMERS Local merchant.
- Added the merchant-owned 20% food + non-alcoholic drink perk with show-your-screen redemption and no insurance condition.
- Added independently attributable QR campaigns for Stevie's table, counter, and placard surfaces.
- Added a simple print placard, merchant staff redemption guide, public-identity validation note, pilot launch manifest, and operational closeout runbook.
- Kept the Auto and Home pilot slots unfilled rather than fabricating participating businesses or terms.
- Preserved existing insurance funnels, CoverageFit, Local attribution runtime, join flow, Worker routing, and the public Local/insurance boundary.
- 408-LOCAL-1.9 is deployable as a one-merchant pilot but remains operationally open until the planned Auto and Home merchants are recruited and validated.

# 408-LOCAL-1.8 — 408FARMERS Site Integration

- Added `Local` to the root 408FARMERS navigation without replacing or outranking the primary Coverage Review and contact actions.
- Added a lower-page 408FARMERS Local module after the professional-program section, introducing Eat & Drink, Home, and Auto discovery plus a separate merchant-pilot join path.
- Added a dedicated Local column to the homepage footer with responsive four-to-two-to-one-column behavior.
- Added post-submission Local value modules to Home, Home + Auto, Healthcare, Teachers, Technology, and Engineers fallback receipts, always after the existing insurance receipt/next-step content.
- Added a homebuyer-specific Local module to the completed Buyer receipt while leaving the active buyer intake untouched.
- Added token-safe site-entry surfaces for the existing 408-LOCAL-1.6 attribution engine; no new analytics event names or PII were introduced.
- Preserved active insurance forms, form transport, CoverageFit, Local merchant data/redemption/join runtimes, Worker routing, insurance economics, and the rule that public Local perks require no insurance quote or purchase.
- Advanced the Local roadmap to 408-LOCAL-1.9 — Pilot Merchant Launch.

# 408-LOCAL-1.7 — Insurance Conversion Bridge

- Added a dedicated optional homeowner insurance module to active Local merchant-perk pages only after the merchant profile, current perk, terms, redemption action, and explicit Local/insurance boundary.
- Replaced the merchant-detail header insurance shortcut with a Local-directory action so the page’s first dedicated insurance conversion invitation occurs after merchant value.
- Added two existing-journey choices: Home + Auto review and Home-only review. No Local lead form, quote gate, or automatic CoverageFit launch was added.
- Reused the certified 408-LOCAL-1.6 attribution engine so bridge clicks carry Local merchant/perk/surface/campaign context into the existing 408FARMERS acquisition flows and emit the existing `insurance_cta_click` event.
- Explicitly tells visitors that the merchant perk is already available and that using or skipping an insurance review does not change the perk, its terms, or redemption.
- Suppresses the dedicated bridge when a public merchant has no active perk, preventing a no-offer merchant page from becoming an insurance acquisition surface.
- Preserved the merchant catalog/schema, merchant join workflow, redemption logic, Local analytics privacy boundary, existing lead capture, CoverageFit zero-repeat handoff, insurance pricing/discount/eligibility behavior, and the no-insurance-condition rule for public Local perks.
- Advanced the Local roadmap to 408-LOCAL-1.8 — 408FARMERS Site Integration.

# 408-LOCAL-1.6 — Local Attribution Engine

- Added `shared/local-attribution.js` with a bounded 30-day, non-PII Local first-touch context for merchant, perk, surface, campaign/variant, and standard UTM attribution.
- Added the five minimum Local events: `local_view`, `merchant_view`, `perk_open`, `perk_redeem_intent`, and `insurance_cta_click`, emitted to `dataLayer`, a `408farmers:local-event` DOM event, and a non-blocking same-origin collector.
- Added `POST /api/local/event` with exact-key JSON validation, same-origin/version checks, token-only context, no identity/address/insurance data, and optional D1 persistence via `LOCAL_ANALYTICS_DB` or existing `LIFE_QUEUE_DB`.
- Added merchant/perk-aware directory link propagation and merchant-detail event context without exposing draft/fixture merchants or changing offer eligibility/redemption.
- Added Local attribution continuity into the existing shared lead form and CoverageFit sender so a later voluntary insurance request can retain `source=local`, merchant/perk/surface origin, campaign/variant, and UTMs without asking the visitor to re-enter them. Explicit new attribution parameters still win.
- Marked existing Local insurance-navigation links for measurement but did not add the dedicated 1.7 insurance conversion module or gate any Local value.
- Preserved the merchant catalog/schema, merchant join PII boundary, no-login/no-insurance-gate rules, all insurance economics, and CoverageFit assessment/scoring behavior.
- Advanced the Local roadmap to 408-LOCAL-1.7 — Insurance Conversion Bridge.

# 408-LOCAL-1.4 — Merchant Perk Detail + Redemption

- Added canonical `/local/{merchant-slug}/` merchant pages through Cloudflare Pages Advanced Mode with trailing-slash normalization and reserved Local infrastructure routes protected.
- Added one reusable catalog-driven merchant detail runtime with merchant image/logo, category, neighborhood, descriptions, website/Instagram/directions links, current perk, terms, and fail-closed inactive/fixture handling.
- Added `Use This Perk` show-your-screen MVP redemption with merchant name, exact current offer, merchant terms, independent-offer language, accessible close behavior, and no account or identity requirement.
- Added directory-to-merchant links while keeping inline directory cards non-redeeming.
- Scheduled, expired, paused, draft, and inactive perks cannot expose redemption; draft/paused/inactive merchants and fixtures remain non-public.
- Preserved the catalog/schema/data-model source, all existing insurance acquisition flows, CoverageFit behavior, and the rule that Local perks are independent of insurance purchase, quote, pricing, discounts, eligibility, underwriting, or coverage.
- Advanced the Local roadmap to 408-LOCAL-1.5 — Merchant Join Flow.

# 408-LOCAL-1.3 — Merchant Discovery Directory

- Turned `/local/` into a data-driven discovery directory consuming the validated 408-LOCAL-1.2 merchant/perk catalog.
- Added All / Eat & Drink / Home / Auto filtering, featured merchant presentation, neighborhood labels, image/logo fallbacks, mobile horizontal category controls, and accessible live directory status.
- Added lifecycle-safe merchant cards: active in-window perks render as current; scheduled, expired, paused, draft, and inactive perks never render as active.
- Kept active merchants discoverable when a perk is temporarily unavailable, while draft/paused/inactive merchants and all fixture records remain hidden.
- Added expandable merchant discovery details without preempting the canonical merchant detail/redemption route reserved for 408-LOCAL-1.4.
- Production remains intentionally free of real merchant claims: the three fixtures are still draft/non-public and `/local/` shows a pilot empty state until approved merchants are loaded.
- Preserved the frozen Local/insurance separation, existing 408FARMERS insurance acquisition behavior, Cloudflare routing, and CoverageFit behavior.
...
- Added the canonical `/local/` public route for 408FARMERS Local.
- Introduced a branded South Bay merchant/perks foundation with Eat & Drink, Home, and Auto pilot categories.
- Made the program boundary explicit: Local perks do not require an insurance purchase, quote, lead submission, or CoverageFit assessment and do not affect insurance pricing, discounts, eligibility, coverage, or underwriting.
- Added a merchant pilot-interest surface without pre-building the later merchant join workflow.
- Added scoped responsive Local styling and reused the existing site-wide accessibility layer.
- Added `/local` canonical directory normalization to Cloudflare Pages Advanced Mode.
- Added `408-LOCAL-ROADMAP.md` with the full 1.x MVP, 2.x network expansion, 3.x homeowner ecosystem, and 4.x merchant-to-commercial roadmap.
- Added sprint contract, release certification, and dedicated Local QA.
- Left all existing 408FARMERS insurance acquisition flows and CoverageFit behavior unchanged.
## 408-LOCAL-1.2 — Merchant Data Model

- Added a versioned normalized Local catalog with separate merchant and perk collections joined by stable IDs.
- Added a machine-readable JSON Schema plus a storage-neutral runtime helper for validation, lifecycle/date resolution, stable `/local/{slug}/` route construction, and reusable merchant view models.
- Added explicit model guardrails preventing draft, paused, inactive, scheduled, or expired perks from resolving active.
- Required independent-offer language on every perk and added validation against common Farmers/408FARMERS merchant-endorsement claims.
- Added three non-public draft fixtures covering Eat & Drink, Home, and Auto and certified that all three render through one reusable renderer without merchant-specific markup.
- Preserved the 408-LOCAL-1.1 public `/local/` page, Local CSS, Worker routing, all insurance acquisition flows, and CoverageFit behavior unchanged.
- Advanced the Local roadmap to 408-LOCAL-1.3 — Merchant Discovery Directory.


## 408-LOCAL-1.5 — Merchant Join Flow

- Added the canonical `/local/join/` South Bay merchant application route and branded receipt page.
- Added structured business, contact, category, location, website/social, proposed-perk, acknowledgment, and notes fields so pilot applications no longer need to be reconstructed from texts or emails.
- Added explicit merchant-facing boundaries: no insurance purchase or quote requirement, no commercial-insurance condition, no promised referrals/lead volume/sales, no automatic directory acceptance, and no merchant endorsement/certification implication.
- Added a dedicated `/api/local/merchant-application` same-origin Cloudflare relay with field allowlisting, validation, honeypot protection, size limits, no PII logging, and optional `LOCAL_MERCHANT_FORMSPREE_ENDPOINT` override.
- Kept a direct Formspree HTML-form fallback for delivery resilience while preserving the existing production endpoint by default.
- Added privacy-safe landing-page and standard UTM capture for the merchant application itself; consumer Local attribution remains owned by 408-LOCAL-1.6.
- Updated the `/local/` business CTA to use the repeatable join flow.
- Preserved the 1.2 catalog, 1.3 discovery directory, 1.4 merchant detail/redemption behavior, all insurance acquisition flows, CoverageFit, and the public no-insurance-gate rule.

## 408-UI-3.3 — Home Conversion Experience

- Re-composed `/home/` into a compact two-column platform hero using the UI-3 navy/red/white hierarchy.
- Converted the Home review area into a contained guided-review shell with cleaner benefit cards and a restrained conversion card.
- Unified quick-question choices, progressive lead fields, progress bars, payoff states, direct-contact utilities, post-lead receipt, and optional CoverageFit invitation styling.
- Reworked the three-step explanation and Dylan/Farmers credibility section into cleaner platform modules.
- Preserved all existing Home form field names, campaign and attribution fields, Formspree submission, address autocomplete, renter routing, post-lead logic, and CoverageFit zero-repeat handoff.
- Preserved `_worker.js`; supported `/home/qr/*`, `/home/campaign/*`, and Home deep routes continue to resolve to the canonical Home asset and automatically receive the new UI.

## 408-UI-3.2.1 — Homepage Conversion Identity Alignment

- Preserved the UI-3.2 navy/red/white platform aesthetic while restoring the original 408FARMERS conversion identity.
- Restored **Insurance That Fits.** and **Start with a Coverage Review. Not a quote.** to the homepage hero.
- Replaced product-first primary selection with a six-path **What brought you here today?** situation chooser.
- Kept Home, Home + Auto, Buyer, and Life as secondary product navigation for visitors who already know what they want.
- Elevated CoverageFit into the hero, situation-to-review bridge, and earlier explanation section.
- Preserved the premium/renewal/homebuying/life-change review triggers.
- Preserved every baseline homepage destination and tracked anchor contract and left UI-3.3 Home, Workers, forms, attribution, Local, Buyer, Life, and CoverageFit runtime unchanged.


## 408-UI-3.4 — Home + Auto Conversion Experience

- Re-composed `/auto-bundle/` into the UI-3 navy/red/white platform conversion language.
- Added a controlled Home + Auto hero with renter relevance preserved and no pre-quote discount promise.
- Converted the existing bundle form into a restrained centered guided-review card with unified fields, focus states, consent treatment, and Farmers-red primary action.
- Simplified duplicate producer content inside the form while retaining the dedicated Dylan/Virginia Tam/Farmers credibility module below.
- Restyled the bundle explanation and three-step path with lighter borders, smaller radii, and reduced shadow weight.
- Preserved field names, required semantics, source/campaign/UTM attribution, Formspree submission, address autocomplete, `housing_context` branching, renter destination, post-lead engagement, optional CoverageFit invitation, and zero-repeat handoff.
- Preserved `_worker.js` and every other public HTML surface from the UI-3.2.1 baseline.

## 408-UI-3.6 — Professional Programs Convergence

- Unified Healthcare, Teachers, Technology, and Engineers under one shared 408FARMERS Professional Programs visual family.
- Added a compact professional-program switcher while retaining each route's original occupation-specific message and eligibility context.
- Re-composed all four professional routes into the UI-3 navy/red/white conversion system with consistent guided-review cards, fields, consent, CTAs, review explanation, three-step progression, and producer credential close.
- Added an explicit visual boundary that professional role is review context rather than an automated eligibility decision.
- Preserved the four lead forms byte-for-byte, including profession choices, Formspree submission, source/campaign/UTM attribution, consent, post-lead behavior, CoverageFit invitation, and zero-repeat handoff.
- Left `_worker.js`, all protected runtime files, and every non-professional public HTML surface unchanged.


## 408-UI-3.10 — Mobile Interaction Pass
- Added `shared/mobile-interaction.css` across all 27 public UI-3 consumer surfaces.
- Added safe-area aware header/navigation/footer/dialog treatment and `viewport-fit=cover`.
- Added coarse-pointer 44px touch ergonomics and 16px mobile form controls.
- Added focus/keyboard scroll margins, phone CTA stacking, horizontal professional/Local touch scrollers, and dynamic viewport constraints.
- Added short-landscape/in-app-browser refinements without hiding consent, warnings, disclosures, or other guardrails.
- Preserved Worker, forms, attribution, Local, Life secure submission, QR routing, and CoverageFit behavior.
- `/life-ops/` remains intentionally excluded as an internal producer workspace.
## 408-UI-3.13.1B — Professional Programs Humanization
- Humanized Healthcare, Teachers, Technology, and Engineers using profession-first heroes, campaign-derived professional photography, restrained warm-gold identity accents, and an early Dylan Human Trust Signature.
- Replaced pill-heavy trust presentation and several nonessential card shells with more editorial hierarchy while preserving Farmers red as the primary conversion color.
- Simplified professional form framing and CTA language without changing controls, required semantics, Formspree, consent, attribution, eligibility logic, Campaign Entry Registry/runtime, or CoverageFit handoff.
- Preserved Life/Life Ops and the 408-INFRA-1.1 Function boundary.
- Next: 408-UI-3.13.1C — Core Insurance Humanization.



## 408-UI-3.13.1D — Relationship + Completion Humanization
- Humanized confirmed post-lead and optional CoverageFit continuation states with truthful first-person Dylan acknowledgement and compact producer imagery.
- Added the explicit `You’re not submitting another request.` safeguard while preserving semantic fields, branching, events, and handoff behavior.
- Humanized Home, Home + Auto, Buyer, Healthcare, Teachers, Technology, and Engineers fallback receipts without changing their direct contact, underwriting, or Local-separation contracts.
- Replaced abstract Contact trust chips with direct-to-Dylan relationship cues while preserving SMS/phone/email destinations.
- Warmed 408FARMERS Local and merchant-join presentation while preserving merchant-owned offers, no-gate separation, Workers, attribution, redemption, and no-guarantee rules.
- Preserved Life/Life Ops, 408-INFRA-1.1, the Campaign Entry Registry, Workers, and eight protected forms.
- Next: 408-UI-3.13.1E — Human Trust Regression + Production Certification.
## 408-UI-3.13.1E — Human Trust Regression + Production Certification
- Added no intentional customer-facing design or runtime changes.
- Froze 185/185 shipped product/runtime files and 8/8 protected forms exactly to the 3.13.1D input baseline.
- Preserved Life/Life Ops and 408-INFRA-1.1 exactly.
- Re-ran the current Human Trust browser/source, cross-journey E2E, production browser, campaign, asset/performance, metadata, JavaScript, link, Worker, routing, static, and Local browser regression matrix.
- Certified the Human Trust branch PASS / DEPLOYABLE-CERTIFIED and froze the current customer-facing UI.
- Added `HUMAN-TRUST-PRODUCTION-CERTIFICATION.md` and `UI3_13_1E_DEPLOYMENT_SMOKE_RUNBOOK.md`; deployed-domain/external-service smoke remains required after upload.
- LOCAL-1.10 planned three-merchant pilot operational NO-GO remains unchanged.


## 408-UI-4.1 — Editorial Platform Foundation
- Added non-Life editorial composition foundation and warm-gold accent tokens.
- Added split hero, contextual photography, action-panel, Dylan relationship-band, editorial-column, step, and trust-strip primitives.
- Added non-Life header refinement with Professionals navigation and Text/Call Dylan actions.
- Added full UI-4.1–4.10 roadmap and editorial design-system documentation.
- Preserved all non-Life body markup, Life/Life Ops, Workers, forms, attribution, Local, CoverageFit, and 408-INFRA-1.1 behavior.

## 408-UI-4.2 — Homepage Editorial Convergence
- Rebuilt the homepage composition around the UI-4 editorial proposition / context photography / situation-first action-panel grammar.
- Preserved “Insurance That Fits.” and “Start with a Coverage Review. Not a quote.”
- Added the full Dylan relationship band below the first conversion section.
- Retained every certified homepage destination and tracked anchor from the UI-4.1 input.
- Flattened lower homepage sections to reduce dashboard/SaaS chrome while preserving CoverageFit, Professional Programs, Local, and agency context.
- Corrected homepage UI-3/UI-4 runtime order so the shared editorial header enhancement runs after the universal header exists.

## 408-UI-4.3 — Home + Bundle Editorial Convergence
- Recomposed `/home/` and `/auto-bundle/` into the UI-4 editorial copy / contextual image / working panel grammar.
- Preserved both `#leadForm` blocks exactly from the UI-4.2 input.
- Preserved Home campaign/flyer/QR message-match nodes and Bundle campaign-entry nodes.
- Added Dylan relationship bands, editorial lower-page columns, trust strips, and route-specific responsive rules.
- Added a context-only crop of the existing Bundle creative so embedded campaign typography no longer appears in the website image zone.
- Life, Workers, routing, attribution, post-lead, CoverageFit, and 408-INFRA-1.1 remain protected.

## 408-UI-4.4 — Buyer Editorial Convergence
- Re-composed `/buyer/` into the UI-4 editorial/photo/action-panel system while preserving the certified Buyer form and all referral/closing behavior.


## 408-UI-4.5 — Professional Programs Campaign Convergence
- Re-composed all four Professional Programs routes around the approved UI-4 editorial/photo/action-panel mockup grammar.
- Preserved the Healthcare, Teachers, Technology, and Engineers `#leadForm` blocks exactly from UI-4.4.
- Added stronger selective warm-gold profession headline treatment, prominent shared profession navigation, context crops from existing campaign assets, Dylan relationship bands, and editorial lower-page support columns.
- Preserved occupation eligibility qualifiers, Campaign Message-Match, Formspree, consent, attribution, CoverageFit, Life/Life Ops, Workers, and 408-INFRA-1.1.

## 408-UI-4.6 — Local Community Convergence
- Re-composed Local into the UI-4 editorial/community grammar using only real supported categories and catalog data.
- Preserved Local merchant/perk/redemption/attribution/join behavior and explicit insurance separation.
- Preserved LOCAL-1.10 operational NO-GO pending real Auto + Home merchants.


## 408-UI-4.10 — Functional Regression + Production Certification
- Added no intentional customer-facing or runtime change.
- Froze 223/223 deployable/runtime product files exactly to the 408-UI-4.9 input.
- Certified Life/Life Ops and shipped Life assets/runtime 14/14 exact, INFRA boundary files 4/4 exact, and critical handoff/attribution/data files 12/12 exact.
- Added the 559/559 UI-4.10 functional/production source-freeze gate and a machine-readable regression matrix.
- Re-ran the current UI-4.9/4.8/4.7, cross-journey E2E, static, metadata, link, JavaScript, Home routing, Local Worker, Life browser, CoverageFit runtime, and INFRA regression gates.
- Preserved the UI-4.9 Home 330 KiB source-budget exception; the historical UI-3.13 asset suite remains 98/99 solely on its superseded 300 KiB Home threshold.
- Recorded legacy browser/source harness incompatibilities as non-gating historical exceptions rather than restoring obsolete selectors or top-level VERSION values.
- Added `UI4_10_DEPLOYMENT_SMOKE_RUNBOOK.md` and closed UI-4 as PASS / DEPLOYABLE-CERTIFIED / CUSTOMER-FACING PRESENTATION FROZEN.
